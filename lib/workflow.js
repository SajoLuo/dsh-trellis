 ...
 *
 * Canonical Trellis semantics are session-pointer-driven; dsh sub-agents
 * (and fresh sessions) do not share the main session's pointer, so the
 * resolver falls back to a single-active-task scan of `.trellis/tasks/`
 * (preferring `in_progress` over `planning` by mtime). That scan is what
 * keeps the recursion-guard breadcrumb visible on sub-agent turns — the
 * equivalent of Codex's SubagentStart context injection — and gives fresh
 * sessions an auto-attach resume path. A task leaves the scan when archived
 * (`task.py archive`) or finished + archived.
 *
...
export async function resolveActiveTask(root, contextKey, fs) {
  ...
  // 2. Single-active-task scan fallback.
