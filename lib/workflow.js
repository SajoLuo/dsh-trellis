...
const STATUS_RE = /\[workflow-state:([A-Za-z0-9_-]+)\]([\s\S]*?)\[\/workflow-state:\1\]/g;
 ...
 *
 * Order: (1) the session-scoped pointer file `.trellis/.runtime/sessions/<key>.json`
 * whose name matches the caller-supplied context key, (2) any dsh-created pointer
 * (latest `last_seen_at`), (3) global scan of `.trellis/tasks/<task-name>/task.json`
 * preferring `in_progress` over `planning` by mtime.
 *
...
export async function resolveActiveTask(root, contextKey, fs) {
  ...
  for (const name of names) {
    ...
    if (data === null || typeof data !== "object" || typeof data.current_task !== "string") continue;
    pointerCandidates.push({
      name,
      taskPath: data.current_task,
      platform: typeof data.platform === "string" ? data.platform : null,
      lastSeen: typeof data.last_seen_at === "string" ? data.last_seen_at : "",
    });
  }
  const own = pointerCandidates.find((p) => p.name === `${contextKey}.json`);
  if (own) {
    const status = await readTaskStatus(root, own.taskPath, fs);
    if (status !== null) return { path: own.taskPath, status };
  }
  const dshPointers = pointerCandidates
    .filter((p) => p.platform === "dsh")
    .sort((a, b) => b.lastSeen.localeCompare(a.lastSeen));
  for (const pointer of dshPointers) {
    const status = await readTaskStatus(root, pointer.taskPath, fs);
    if (status !== null) return { path: pointer.taskPath, status };
  }

  // 2. Global task scan fallback.
  const tasksDir = join(root, ".trellis", "tasks");
  let taskDirs = [];
  try {
    taskDirs = await fs.listDir(tasksDir);
  } catch {
    taskDirs = [];
  }
  const candidates = [];
  for (const name of taskDirs) {
    ...
    const taskJson = join(tasksDir, name, "task.json");
    let data = null;
    try {
      data = JSON.parse((await fs.readFile(taskJson)) ?? "null");
    } catch {
      continue;
    }
    ...
    const info = await fs.stat(taskJson);
    candidates.push({
      path: `.trellis/tasks/${name}`,
      status,
  ...
  return winner === undefined ? null : { path: winner.path, status: winner.status };
}
...
export async function readTaskStatus(root, taskPath, fs) {
  try {
    const data = JSON.parse((await fs.readFile(join(root, taskPath, "task.json"))) ?? "null");
    if (data !== null && typeof data === "object" && typeof data.status === "string") {
      return data.status;
    }
    return null;
  } catch {
 ...
 */
export async function resolveTrellisState({ cwd, markers, contextKey, fs, cache }) {
  ...
  const digest = createHash("sha1").update(`${status}\n${task?.path ?? ""}\n${body}`).digest("hex");
