...
/** Read one session event across the current and legacy DSH session APIs. */
export function sessionEventAt(session, seq) {
  if (typeof session.eventAt === "function") return session.eventAt(seq);
  if (typeof session.events?.get === "function") return session.events.get(seq);
...
export function isBreadcrumbVisible(agent, digest) {
  ...
  // Current DSH sessions, including Session V3, expose an immutable on-demand
  // snapshot. Keep the legacy collection fallback for older versions covered
  // by our peer range.
  const events =
    typeof session.snapshotEvents === "function"
      ? session.snapshotEvents()
      : session.events;
  for (const [fallbackSeq, event] of events.entries()) {
    const seq = event?.seq ?? fallbackSeq;
...
export function hasSkipKeyword(messages, keyword) {
  ...
  for (const message of messages) {
    ...
    for (const part of parts) {
      if (typeof part?.text === "string" && pattern.test(part.text))
        return true;
    }
  }
  return false;
 ...
 */
export function createBreadcrumbComposer(resolved, ctx, dependencies = {}) {
  ...
  const compose = async (agent, messages, signal) => {
    ...
    const contextKey =
      sessionId === "" ? "dsh" : `dsh_${sanitizeContextKey(sessionId)}`;
