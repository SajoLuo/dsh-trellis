...
export function apply(ctx, config = {}) {
  ...
  // Once rc.8's settings service appears, its resolved section becomes the
  // live source and a saved Web card edit remounts only this plugin's runtime.
  ctx.effect(() => {
    mountRuntime(config);
    return () => {
      stopped = true;
  ...
  // Optional by design: rc.6 profiles have no settings service and retain the
  // loader-config behavior, while rc.8 Web profiles expose this namespace.
  ctx.inject(["settings"], (settingsCtx) => {
    ...
    const adopt = () => mountRuntime(scope.get());
    adopt();
    const unwatch = scope.watch(adopt);
    settingsCtx.effect(
      () => () => {
        unwatch();
        mountRuntime(config);
      },
      "dsh-trellis.settings",
    );
  });
}
