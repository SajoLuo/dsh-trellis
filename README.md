...
2. **原生会话上下文身份** — 直接使用 DSH 自动注入每个 agent shell 的 `DSH_SESSION_ID`，让 `task.py start/create/current` 解析到会话级 active-task 指针；插件命令启动的子进程也转发同一个原生 ID，不另造身份层。
3. **原生子代理同步** — Trellis 角色默认用 DSH continuable 后台子代理；主会话先继续独立工作，耗尽后调用事件驱动的 `trellis_wait`。它监听 DSH 的 `subagent/end`，只在原生 settlement notice 已排入父会话后返回，不用 shell sleep、轮询或 `job_output`。
...
- **会话身份**：DSH 原生提供 `DSH_SESSION_ID = agent.session.header.id`；Trellis 在读指针时统一做 `dsh_` 前缀和 sanitize。主会话与子代理各自保留 DSH 的隔离身份，子代理通过派发 prompt 首行的 `Active task:` 和角色 prelude 取得父任务上下文。
...
- **子代理并发**：角色派发由 Trellis workflow 使用 DSH 原生 continuable `subagent`。主会话并行做独立工作；需要汇合时用 `trellis_wait <subagent_id>` 等待 DSH 原生结算事件。只有派发当下就没有任何独立工作、下一步又立刻依赖结果时，才显式使用前台模式。
